...
tanhSerialize :: Monad m => conf -> (Get (m ()), () -> m Put)
tanhSerialize _ =
  ( pure $ pure (),
    const $ pure $ pure ()
  )

debugChangeMx mx1 mx2 = do
  v <- head . head . head <$> mxToLists mx1
  v2 <- head . head . head <$> mxToLists mx2
  liftIO $ printf "in: %.2f   out: %.2f\n" v v2
...
tanAvg :: Monad m => (() -> () -> m (), () -> Int -> m ())
tanAvg = (const (const (pure ())), const (const (pure ())))
...
tanH _ = Layer tanF tanB tanAvg tanUpd tanInit tanhSerialize
